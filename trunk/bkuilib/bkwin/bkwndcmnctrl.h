...
//////////////////////////////////////////////////////////////////////////
//Image Slider
class CBkImageSlider : public CBkWindow
{
	BKOBJ_DECLARE_CLASS_NAME(CBkImageSlider, "slider")
public:
	CBkImageSlider()
	{
		m_pSkinBg = NULL;
	...
	LRESULT OnNcCalcSize(BOOL bCalcValidRects, LPARAM lParam)
	{
		...
		if (m_pSkinItem)
		{
			SIZE sizeBg = m_pSkinItem->GetSkinSize();
	...
	void OnPaint(CDCHandle dc)
	{
		...
		HPEN hpenOld = dc.SelectPen(penFrame);
		BKDLG_POSITION pos;
		this->GetDlgPosition(&pos);
		int y_offset = m_rcWindow.Height() / 4;
		CRect poss_rc( m_rcWindow );
		poss_rc.top += y_offset;
		poss_rc.bottom -= y_offset;
		//poss_rc.DeflateRect( 0 , y_offset , 0 , y_offset );

		/*if (m_pSkinBg)
		{
		m_pSkinBg->Draw(dc, poss_rc, BkWndState_Normal);
		}
		else*/
		{
			dc.RoundRect(poss_rc, CPoint(2, 2));
		}
		...
		CRect rcItem;
		rcItem.top = m_rcWindow.top;
		rcItem.bottom = m_rcWindow.bottom;
		if( rcPosBar.Width() < m_pSkinItem->GetSkinSize().cx )
		{
			rcItem.left = rcPosBar.left;
		}
		else if ( (m_rcWindow.right - rcPosBar.right) <  m_pSkinItem->GetSkinSize().cx )
		{
			rcItem.left = m_rcWindow.right - m_pSkinItem->GetSkinSize().cx;
		}
		else
		{
			rcItem.left = rcPosBar.right - (m_pSkinItem->GetSkinSize().cx / 2);
		}

		rcItem.right = rcItem.left + m_pSkinItem->GetSkinSize().cx;
		if ( m_pSkinItem )
		{
			m_pSkinItem->Draw( dc, rcItem, -1 );
		}
	...
	void OnLButtonDown(UINT nHitTest, CPoint point)
	{
		m_bLBDown = TRUE;
		//this->m_hBkWndParent
		//::SetCapture( GetContainer() );
	...
	{
		m_bLBDown = FALSE;
		//HWND hwnd = this->GetContainer();
		//HWND hh = ::GetParent( hwnd );
		//::ReleaseCapture();
		//_TrackMouseEvent();
		DWORD val = CalcDrawVal( (int)point.x );
	}

	void OnMouseMove(UINT nFlags, CPoint point)
	{
		DWORD escTime;
		if( m_bLBDown &&  ( escTime = GetTickCount() - m_dwTime ) > 300 )
		{
			//drag			
			CalcDrawVal((int)point.x);
			HWND hwnd = GetContainer();
			
		}
	}

	DWORD CalcDrawVal( int x  )
	{
		DWORD m_dwOldVal = m_dwValue;
		int len = x - m_rcWindow.left;
		float fval = (float)len / (float)m_rcWindow.Width();
		this->m_dwValue = m_dwMaxValue * fval;
		NotifyInvalidateRect( m_rcWindow );
		if ( m_dwValue != m_dwOldVal )
		{
			SendMessage( ::GetParent( GetContainer() ), BK_VALUE_CHANGE , this->m_uCmdID , m_dwValue );	
		}
		return m_dwValue;
	}
...
protected:
	...
	BKWIN_DECLARE_ATTRIBUTES_BEGIN()
		BKWIN_SKIN_ATTRIBUTE("bgskin", m_pSkinBg, TRUE)
		BKWIN_SKIN_ATTRIBUTE("posskin", m_pSkinPos, TRUE)
		BKWIN_SKIN_ATTRIBUTE("itemskin", m_pSkinItem, TRUE)
		BKWIN_DWORD_ATTRIBUTE("min", m_dwMinValue, FALSE)
		BKWIN_DWORD_ATTRIBUTE("max", m_dwMaxValue, FALSE)
		BKWIN_DWORD_ATTRIBUTE("value", m_dwValue, FALSE)
	BKWIN_DECLARE_ATTRIBUTES_END()
